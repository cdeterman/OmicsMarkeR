...
plsDA <-
           ...
           learn = NULL, test = NULL, cv = "LOO", k = NULL, retain.models = FALSE)
  {
    ...
    if (validation %in% c("none","crossval")) {
      get_plsda = my_plsDA(X, y, 1:n, 1:n, autosel, comps, cv=cv, k=k, retain.models = retain.models)
    ...
    if (validation == "learntest")
    {
      ...
      # apply plsDA
      get_plsda = my_plsDA(X, y, learn, test, autosel, comps, retain.models = retain.models)
