...
#' @description Optimizes each model via LOO CV based upon the parameters provided either by the internal \code{\link{denovo.grid}}
...
)
{
  ...
  # set number of iterations to number of resample lists (i.e. number of samples)
  ...
  tmp.list <- foreach(algo = seq(along = method),
            ...
            .errorhandling = "stop") %op%
    {
      ...
      # create models
      mod <- training(data = trainData[complete.cases(trainData[inTrain[[iter]],,drop = FALSE]),,drop = FALSE],
                      method = method[algo],
      ...
      predicted <- predicting(method = method[algo],
                              modelFit = mod$fit,
                              orig.data = trainData,
  ...
  if(length(method) > 1){
    ...
    par_levs <- lapply(tunes, FUN = function(x) unique(sapply(x[,!colnames(x) %in% c("pred", "obs", "sampleIndex"), drop = F], as.factor)))
    ...
    for(im in 1:length(method)){
      ...
      split.results[[im]] <- do.call("rbind", lapply(split(tunes[[im]], tunes[[im]][,c(filter)]),
                                                     FUN = perf.calc,
                                                     lev = lev))  
      # Make sure parameters are numeric for subsequent sorting
      # Bind parameters to results
      # Rename rows to make pretty
      split.results[[im]] <- cbind(sapply(as.data.frame(par_levs[[im]]), FUN = function(x) as.numeric(as.character(x))), split.results[[im]])
    ...
    par_levs <- unique(sapply(tunes[,!colnames(tunes) %in% c("pred", "obs", "sampleIndex"), drop = F], as.factor))
    ...
    split.results <- do.call("rbind", lapply(split(tunes, tunes[,c(filter)]),
                                             FUN = perf.calc,
                                             lev = lev))
    split.results <- cbind(sapply(as.data.frame(par_levs), FUN = function(x) as.numeric(as.character(x))), split.results)
