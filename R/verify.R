...
verify <- function(x, y, method, f, stability.metric,
                   model.features, na.rm = na.rm, no.fs){  
    
    # set method to lowercase to avoid user frustration
    method <- tolower(method)
    
    if(!all(method %in% c("plsda","glmnet","gbm", "pam", "rf", "svm")))
        simpleError("Selected method is not plsda, glmnet, gbm, pam, 
                    rf or svm!")
    ...
    if(!no.fs){
        ...
        if(stability.metric == "kuncheva"){
            if(is.null(f)){
                stop("\n Error: Kuncheva requires same number of features 
