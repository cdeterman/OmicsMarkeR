...
library("OmicsMarkeR")
set.seed(123)
dat.discr <- create.discr.matrix(create.corr.matrix(create.random.matrix(nvar = 50, nsamp = 100, st.dev = 1, perturb = 0.2)))
@
...
results <- fs.stability(vars, groups, method = c("plsda", "rf"), f = 20, k = 3, k.folds = 10, verbose = TRUE)
@
